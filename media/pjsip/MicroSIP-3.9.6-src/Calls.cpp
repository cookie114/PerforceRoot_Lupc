...
BOOL Calls::OnInitDialog()
{
	...
	list->InsertColumn(1,Translate(_T("Date")),LVCFMT_LEFT, accountSettings.callsWidth1>0?accountSettings.callsWidth1:71);
...
void Calls::Insert(Call *pCall)
{
	...
	CTime t(pCall->time) ;
	list->SetItemText(i,1,t.Format(_T("%m/%d %H:%M")));
	list->SetItemText(i,2,GetDuration(pCall->duration));
