...
#define SUPPORT_ARGB_1555

#ifdef SUPPORT_ARGB_1555
gint global_Bpp;

#define GST_VIDEO_CAPS_ARGB_1555                              \
    "video/x-raw-rgb, "                                                 \
    "bpp = (int) 16, "                                                  \
    "depth = (int) 16, "                                                \
...
static GstStaticPadTemplate src_template_factory =
    ...
    GST_PAD_ALWAYS,
#ifdef SUPPORT_ARGB_1555
   GST_STATIC_CAPS (GST_VIDEO_CAPS_YUV ("AYUV") ";" GST_VIDEO_CAPS_ARGB";"GST_VIDEO_CAPS_ARGB_1555)
#else
...
static void
#ifndef SUPPORT_ARGB_1555
gst_text_render_render_pangocairo (GstTextRender * render)
#else
gst_text_render_render_pangocairo (GstTextRender * render, int Bpp)
#endif
{
  cairo_t *cr;
...
#else
{
  ...
  render->text_image = g_realloc (render->text_image, Bpp * width * height);
...
#ifndef SUPPORT_ARGB_1555
static void gst_text_render_check_argb (GstTextRender * render)
#else
static void gst_text_render_check_argb (GstTextRender * render, GstCaps **pcaps)
#endif
{
  GstCaps *peer_caps;
...
//  mycaps = gst_pad_get_caps_reffed (render->srcpad);
//  GST_DEBUG_OBJECT (render, "\nmycaps:%s", gst_caps_to_string(mycaps));

  GST_DEBUG_OBJECT (render, "\nhello2222");
  
//  peer = GST_PAD_PEER (render->srcpad);
//  peercaps = gst_pad_get_caps_reffed (peer);
//  gst_object_unref (peer);
//  GST_DEBUG_OBJECT (render, "\npeercaps:%s", gst_caps_to_string(peer_caps));
#endif
  ...
  if (G_LIKELY (peer_caps)) {
    ...
    GST_DEBUG_OBJECT (render, "peer allowed caps (%u structure(s)) are %s",n, gst_caps_to_string(peer_caps));
    ...
    gst_structure_get_int (structure, "width", &render->width);
    gst_structure_get_int (structure, "height", &render->height);

    GST_DEBUG_OBJECT (render, "peer caps width is %d, heigh is %d",render->width,render->height);
...
#else
  gst_text_render_check_argb (render,NULL);
#endif
...
gst_text_renderer_image_to_argb (GstTextRender * render, guchar * pixbuf,
    ...
    int xpos, int ypos, int stride)
#else
    int xpos, int ypos, int stride, int Bpp)
#endif
{
  int i, j;
  guchar *p, *bitp;
  ...
  switch(Bpp) {
	 ...
	 case 2: {
	 	...
	 	guint i,tmp;
		guchar q[4];
		guint16 *ptext;//		static gboolean flag=TRUE;
		//convert 32bit argb to 16bit argb
	  	bitp = render->text_image;
	  	ptext = g_malloc (2 * width * height);

		for(i=0; i< width*height; i++) {
			q[0] = bitp[CAIRO_ARGB_A];
			q[1] = bitp[CAIRO_ARGB_R];
			q[2] = bitp[CAIRO_ARGB_G];
			q[3] = bitp[CAIRO_ARGB_B];
			
			ptext[i] = ((q[0]&0x80)<<15)|((q[1]&0xf8)<<10)|((q[2]&0xf8)<<5)|(q[3]&0xf8);
		...
		render->text_image = ptext;
#endif
		for (i = 0; i < height; i++) {
			p = pixbuf + (ypos + i) * stride + xpos * 2;
			for (j = 0; j < width; j++) {
				p[0] = bitp[0];
				p[1] = bitp[1];
...
#else
{
	...
	gst_text_render_check_argb (render, &caps);
	
	structure = gst_caps_get_structure (caps, 0);
	gst_structure_get_int (structure, "bpp", &bpp);
	if( gst_structure_get_int (structure, "alpha_mask", &alpha_mask) )
		have_alpha = TRUE;
	
	gst_text_render_render_pangocairo (render,(bpp/8));
	global_Bpp = (bpp/8);
	
	if( (bpp == 16) && (have_alpha) ) {
		gst_caps_set_simple (caps, "framerate", GST_TYPE_FRACTION, 1, 1,NULL);
		gst_caps_set_simple (caps, "pixel-aspect-ratio", GST_TYPE_FRACTION, 1, 1, NULL);
    		GST_DEBUG_OBJECT (render, "\ncaps  are %s", gst_caps_to_string(caps));
...
#else
      render->width * render->height * (bpp/8), caps, &outbuf);
#endif
  GST_DEBUG ("Allocating buffer ret = %d, use_ARGB = %d,bpp=%d", ret,render->use_ARGB,bpp);
...
#else
    memset (data, 0, render->width * render->height * (bpp/8));
#endif
  } else {
  ...
  switch (prop_id) {
    ...
    case PROP_FONT_DESC:
    {
      ...
      if (desc) {
        ...
        gst_text_render_render_pangocairo (render);
#else
        gst_text_render_render_pangocairo (render,global_Bpp);
#endif
