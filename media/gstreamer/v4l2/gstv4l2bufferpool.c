 ...
 */
GstV4l2BufferPool *
gst_v4l2_buffer_pool_new (GstElement * v4l2elem, gint fd, gint num_buffers, guint32 phys_base, 
    GstCaps * caps, gboolean requeuebuf, enum v4l2_buf_type type)
{
  ...
  for (n = 0; n < num_buffers; n++) {
    ...
    GST_BUFFER_CAST(pool->buffers[n])->_gst_reserved[0] = pool->buffers[n]->vbuffer.m.offset + phys_base;
