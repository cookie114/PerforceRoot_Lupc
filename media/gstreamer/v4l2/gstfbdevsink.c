...
GST_DEBUG_CATEGORY_STATIC (gst_fbdev_sink_debug);
#define GST_CAT_DEFAULT gst_fbdev_sink_debug
...
gst_fbdevsink_getcaps (GstBaseSink * bsink)
{
	...
	uint32_t alen,rlen,blen,glen;
	uint32_t aoff,roff,boff,goff;
//	uint32_t abyte,rbyte,gbyte,bbyte;
//	gchar * string=NULL;
	...
	alen = fbdevsink->varinfo.transp.length;
	rlen = fbdevsink->varinfo.red.length;
	glen = fbdevsink->varinfo.green.length;
	blen = fbdevsink->varinfo.blue.length;
	if(alen > 0)
		have_alpha = TRUE;

	aoff = fbdevsink->varinfo.transp.offset;
	roff = fbdevsink->varinfo.red.offset;
	goff = fbdevsink->varinfo.green.offset;
	boff = fbdevsink->varinfo.blue.offset;
	...
	switch (fbdevsink->varinfo.bits_per_pixel) {
		...
		case 32:
			...
			if (have_alpha)
				string = __GST_VIDEO_CAPS_MAKE_32A (rbyte, gbyte, bbyte, abyte);
			else
				string = __GST_VIDEO_CAPS_MAKE_32 (rbyte, gbyte, bbyte);
...
//			caps = gst_caps_from_string (string);
	...
	gst_caps_set_simple (caps,
		...
		"width", G_TYPE_INT, fbdevsink->varinfo.xres,
		"height", G_TYPE_INT, fbdevsink->varinfo.yres,
		NULL);

	if (have_alpha)
		gst_caps_set_simple (caps, "alpha_mask", G_TYPE_INT, amask, NULL);
#if 1	
{
	GstPad * pad = bsink->sinkpad;
	//  	GstCaps *caps = GST_PAD_CAPS(pad);
	GstCaps *templ_caps = GST_PAD_TEMPLATE_CAPS (GST_PAD_PAD_TEMPLATE (pad));
      GST_DEBUG
          ( "\npad returned caps: %s" 
            "\ntemplate caps: %s",
          gst_caps_to_string(caps), gst_caps_to_string(templ_caps));
}
#endif
	return caps;
}
...
gst_fbdevsink_setcaps (GstBaseSink * bsink, GstCaps * vscapslist)
{
  ...
  GST_DEBUG ("WxH = %dx%d", fbdevsink->width, fbdevsink->height);
...
//  static boolean flag=TRUE;
  ...
  fbdevsink = GST_FBDEVSINK (bsink);
#if 0
//initialization: set alpha = 0
  if(flag) {
  	memset(fbdevsink->framebuffer,0, fbdevsink->width*fbdevsink->height * fbdevsink->bytespp);
	flag = FALSE;
  }
#endif

  GST_DEBUG ("framebuffer = 0x%08x,lines=%d,cy=%d", fbdevsink->framebuffer,fbdevsink->lines,fbdevsink->cy);
  GST_DEBUG ("line_length = %d,cx=%d,bytespp=%d", fbdevsink->fixinfo.line_length,fbdevsink->cx,fbdevsink->bytespp);
  GST_DEBUG ("width = %d,linelen=%d", fbdevsink->width,fbdevsink->linelen);
...
gst_fbdevsink_class_init (GstFBDEVSinkClass * klass)
{
  ...
  GST_DEBUG_CATEGORY_INIT (gst_fbdev_sink_debug, "fbdevsink", 0,
      "fbdevsink element");
