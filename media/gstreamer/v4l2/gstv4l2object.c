      ...
      if (structure) {
        if (gst_v4l2_formats[i].dimensions) {
          gst_structure_set (structure,
              ...
              "framerate", GST_TYPE_FRACTION_RANGE, 0, 1, 1000000, 1, NULL);
        }
...
return_data:
  ...
  if (G_IS_VALUE (&rates)) {
    ...
    gst_structure_set (s, "framerate", GST_TYPE_FRACTION_RANGE, 0, 1, 1000000, 1,
        NULL);
  }
  return s;
...
gboolean
gst_v4l2_object_get_format (GstV4l2Object * v4l2object, guint32* pixelformat,
    guint32* width, guint32* height, guint32* sizeimage,guint32*buf_addr,guint32*buf_size)
{
  gint fd = v4l2object->video_fd;
  struct v4l2_format format;
  ...
  *sizeimage = format.fmt.pix.sizeimage;
  ...
  if (!gst_v4l2_get_attribute (v4l2object, V4L2_CID_PRIVATE_BASE + 0, buf_addr)) {
    goto get_fmt_failed;
  }
  if (!gst_v4l2_get_attribute (v4l2object, V4L2_CID_PRIVATE_BASE + 1, buf_size)) {
    goto get_fmt_failed;
  }
  return TRUE;
