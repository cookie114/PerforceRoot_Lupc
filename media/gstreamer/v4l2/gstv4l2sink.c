...
#define DRIVER_PROVIDE_ADDR_AND_SIZE
enum
{
  PROP_0,
...
gst_v4l2sink_class_init (GstV4l2SinkClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_PHYS_BASE,
      g_param_spec_uint ("phys-base", "Physical base",
          "The physical address base of the frame buffers",
          0, G_MAXUINT, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  ...
  if (!gst_v4l2_object_set_property_helper (v4l2sink->v4l2object,
    ...
    switch (prop_id) {
      ...
      case PROP_OVERLAY_HEIGHT:
        ...
        break;
      case PROP_PHYS_BASE:
        v4l2sink->phys_base = g_value_get_uint (value);
        break;
      default:
  ...
  if (!gst_v4l2_object_get_property_helper (v4l2sink->v4l2object,
    ...
    switch (prop_id) {
      ...
      case PROP_OVERLAY_HEIGHT:
        ...
        break;
      case PROP_PHYS_BASE:
        g_value_set_uint (value, v4l2sink->phys_base);
        break;
      default:
        G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
        break;
...
gst_v4l2sink_set_caps (GstBaseSink * bsink, GstCaps * caps)
{
  ...
  if (!gst_v4l2sink_set_output (v4l2sink, format->pixelformat, w, h, fps_n,
          fps_d)) {
    ...
    GstCaps * caps, GstBuffer ** buf)
{
  ...
  guint32 pixelformat,  width,  height,  sizeimage;
...
#ifdef DRIVER_PROVIDE_ADDR_AND_SIZE
	if (!gst_v4l2_object_get_format (v4l2sink->v4l2object, &pixelformat, &width, &height,&sizeimage,&v4l2sink->phys_base, &v4l2sink->phys_size) ) {
		return GST_FLOW_ERROR;
	}
	v4l2sink->num_buffers = v4l2sink->phys_size/sizeimage;
	if (v4l2sink->num_buffers > 16) {
		v4l2sink->num_buffers = 16;
	} 
#endif
	printf( "v4l2sink->num_buffers<%d>\n",v4l2sink->num_buffers);

      if (!(v4l2sink->pool = gst_v4l2_buffer_pool_new (GST_ELEMENT (v4l2sink),
                  v4l2sink->v4l2object->video_fd,
                  v4l2sink->num_buffers, v4l2sink->phys_base, caps, FALSE,
...
#if 1
  while (gst_v4l2_buffer_pool_available_buffers (v4l2sink->pool) > 1) {
  //start = gst_util_get_timestamp ();
    GstV4l2Buffer *v4l2buf = gst_v4l2_buffer_pool_dqbuf (v4l2sink->pool);
  //end = gst_util_get_timestamp ();
  //g_print ("a:dq_time=%" GST_TIME_FORMAT " \n",  GST_TIME_ARGS (end - start));
    ...
    if (G_LIKELY (v4l2buf)) {
      ...
      *buf = GST_BUFFER (v4l2buf);
      printf( "<%d>:allocated buffer: res0[%p]data[%p]size[%d]idx[%d]ref[%d]\n",size, GST_BUFFER_CAST(v4l2buf)->_gst_reserved[0], GST_BUFFER_DATA (v4l2buf),GST_BUFFER_SIZE (v4l2buf),v4l2buf->vbuffer.index,GST_MINI_OBJECT_REFCOUNT_VALUE (*buf));
      return GST_FLOW_OK;
    } else {
...
printf("render_buf_a: data[%p]idx[%d]ref[%d]\n",GST_BUFFER_DATA (buf), GST_V4L2_BUFFER (buf)->vbuffer.index,GST_MINI_OBJECT_REFCOUNT_VALUE (buf)); 
    GstFlowReturn ret;
...
printf("render_buf_b: data[%p]idx[%d]ref[%d]\n",GST_BUFFER_DATA (buf), GST_V4L2_BUFFER (buf)->vbuffer.index,GST_MINI_OBJECT_REFCOUNT_VALUE (buf));  
  }
  ...
  //start = gst_util_get_timestamp ();
    GstV4l2Buffer *v4l2buf = gst_v4l2_buffer_pool_dqbuf (v4l2sink->pool);
  //end = gst_util_get_timestamp ();
  //g_print ("b:dq_time=%" GST_TIME_FORMAT " \n",  GST_TIME_ARGS (end - start));
  ...
  g_print ("render_delay=%" GST_TIME_FORMAT " \n",  GST_TIME_ARGS (end - start));
