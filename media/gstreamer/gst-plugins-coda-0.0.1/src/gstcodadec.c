...
GST_DEBUG_CATEGORY_STATIC (codadec_debug);
#define GST_CAT_DEFAULT codadec_debug
...
static void gst_coda_dec_base_init (GstCodaDecClass * g_class);
static void gst_coda_dec_class_init (GstCodaDecClass * klass);
static void gst_coda_dec_init (GstCodaDec * codadec);
#if 0
static void gst_coda_dec_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_coda_dec_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);
#endif
static GstFlowReturn gst_coda_dec_chain (GstPad * pad, GstBuffer * buffer);
static gboolean gst_coda_dec_setcaps (GstPad * pad, GstCaps * caps);
static gboolean gst_coda_dec_sink_event (GstPad * pad, GstEvent * event);
static gboolean gst_coda_dec_src_event (GstPad * pad, GstEvent * event);
static GstStateChangeReturn gst_coda_dec_change_state (GstElement * element,
    GstStateChange transition);
static gboolean gst_coda_dec_src_query (GstPad * pad, GstQuery * query);
static void gst_coda_dec_negotiate (GstCodaDec * dec, gint width, gint height);
static void gst_coda_dec_loop(void *param);
static void gst_coda_dec_frame(GstCodaDec *dec);
static void copydata2buffer(GstCodaDec * dec, guchar *data, guint size, GstClockTime pts);
static void gst_coda_dec_finish(GstCodaDec *dec);
...
GType
gst_coda_dec_get_type (void)
{
  static GType type = 0;

  if (!type) {
    static const GTypeInfo coda_dec_info = {
      sizeof (GstCodaDecClass),
      (GBaseInitFunc) gst_coda_dec_base_init,
      NULL,
      (GClassInitFunc) gst_coda_dec_class_init,
      NULL,
      NULL,
      sizeof (GstCodaDec),
      0,
      (GInstanceInitFunc) gst_coda_dec_init,
    };

    type = g_type_register_static (GST_TYPE_ELEMENT, "GstCodaDec",
        &coda_dec_info, 0);
  }
  return type;
}
...
static void
gst_coda_dec_finalize (GObject * object)
{
  GstCodaDec *dec = GST_CODA_DEC (object);
...
static GstCaps *
gst_coda_vid_caps_new (const char *mimetype, const char *fieldname, ...)
{
...
void
gst_coda_caps_with_codecid (GstCodaDec *dec, enum CodecStd codec_id,const GstCaps * caps)
{
  GstStructure *str;
  const GValue *value;
  const GstBuffer *buf;
...
enum CodecStd
gst_coda_caps_to_codecid (GstCodaDec *dec, const GstCaps * caps)
{
  ...
  gst_coda_caps_with_codecid (dec, handle->BitstreamFormat, caps);
...
static void
gst_coda_dec_base_init (GstCodaDecClass * g_class)
{
  GstElementClass *element_class = GST_ELEMENT_CLASS (g_class);
  GstPadTemplate *sinktempl;//, *srctempl;
  GstCaps *sinkcaps;//, *srccaps;

  gst_element_class_set_details_simple (element_class, "CODA video decoder",
      "Codec/Decoder/Video",
      "Decode Video based on coda core", 
      "wangqin <qin.wang@huayamicro.com>");

  /* get the caps */
  sinkcaps = gst_coda_vid_caps_new ("video/mpeg",
            "mpegversion", GST_TYPE_INT_RANGE, 1, 4, NULL);

  gst_caps_append (sinkcaps, gst_coda_vid_caps_new ("video/x-h264", NULL));
  gst_caps_append (sinkcaps, gst_coda_vid_caps_new ("video/x-wmv", NULL));
  gst_caps_append (sinkcaps, gst_coda_vid_caps_new ("video/x-h263", NULL));
  gst_caps_append (sinkcaps, gst_coda_vid_caps_new ("video/x-msmpeg", 
  	"msmpegversion", GST_TYPE_INT_RANGE, 41, 42, NULL));
  gst_caps_append (sinkcaps, gst_coda_vid_caps_new ("video/x-pn-realvideo", NULL));
  gst_caps_append (sinkcaps, gst_coda_vid_caps_new ("video/x-jpeg", NULL));
  gst_caps_append (sinkcaps, gst_coda_vid_caps_new ("video/x-divx",
  	"divxversion", GST_TYPE_INT_RANGE, 4, 5, NULL));
  gst_caps_append (sinkcaps, gst_coda_vid_caps_new ("video/x-xvid", NULL));
  gst_caps_append (sinkcaps, gst_coda_vid_caps_new ("video/x-3ivx", NULL));
...
static void
gst_coda_dec_class_init (GstCodaDecClass * klass)
{
  ...
  gobject_class->finalize = gst_coda_dec_finalize;

  gstelement_class->change_state = GST_DEBUG_FUNCPTR (gst_coda_dec_change_state);

  GST_DEBUG_CATEGORY_INIT (codadec_debug, "codadec", 0, "CODA decoder");
...
#endif
static void frame_output(GstCodaDec *dec, gshort index)
{
...
//	guint test;
	...
	if (pfrm_ctrl->buffromv4l)
		memcpy((gpointer)(GST_BUFFER_DATA(outbuf)), (gpointer)(hal_paddr_to_vaddr((gpointer)pfrm_ctrl->frm_info[index].frm_buffer.bufY)), dec->outsize);
...
static void alloc_and_copy(GstCodaDec *dec,guchar *sps_pps, guint sps_pps_size,
							guchar *in, guint in_size) 
{
    //gint offset = *poutbuf_size;
//    guchar nal_header_size = 4;//offset ? 3 : 4;
    guchar nalu_h[4] = {0, 0, 0, 1};
...
static gint avc_mp4toannexb_filter(GstCodaDec *dec, guchar *buf, guint buf_size)
{
    struct H264BSContext *ctx = (struct H264BSContext *)dec->priv_data;
    guchar unit_type;
    gint nal_size;
    guint cumul_size = 0;
    guchar *buf_end = buf + buf_size;
    ...
    if (!ctx->sps_pps_data) {
        ...
        guchar *extradata = dec->codec_data+4;
        static guchar nalu_header[4] = {0, 0, 0, 1};
		guint i;
...
static void mp4filter(GstCodaDec *dec, guchar *buf, guint buf_size)
{
	copydata2buffer(dec, buf, buf_size, GST_CLOCK_TIME_NONE);
	printf("size %x, data %x, %x, %x, %x\n", buf_size, buf[0], buf[1], buf[2], buf[3]);
}

static void wmvfilter(GstCodaDec *dec, guchar *buf, guint buf_size)
{
	...
	if (ctx->biCompression == GST_MAKE_FOURCC('W', 'M', 'V', '3') ||
		...
		if ( !ctx->first_frame ){
			...
			copydata2buffer(dec, (guchar*)(&(ctx->SeqData)), 20, GST_CLOCK_TIME_NONE);
		}
		copydata2buffer(dec, (guchar*)(&(ctx->frame_size)), 4, GST_CLOCK_TIME_NONE);
...
void frame_free(GstCodaDec *dec, guchar index)
{
	struct vdec_private *ppriv =(struct vdec_private *)dec->priv;
	DecInstCtl *dec_handle = ppriv->inst_handle;
	Frm_Ctrl *pfrm_ctrl = &ppriv->frm_ctrl;
...
void frm_output_dequeue(GstCodaDec *dec)
{
	printf("in deque\n");
	struct vdec_private *ppriv =(struct vdec_private *)dec->priv;
	Frm_Ctrl *pfrm_ctrl = &(ppriv->frm_ctrl);
...
static void sync_reset(struct coda_sync_ctrl* sync_ctrl)
{
...
static struct sync_block* sync_request(struct coda_sync_ctrl* sync_ctrl)
{
	struct sync_block* sync_block;
	if (sync_ctrl->cnt == CODA_META_NUM) {
        GST_LOG("sync_block runout!\n");
		return NULL;
	}
	sync_block =  &(sync_ctrl->block[sync_ctrl->wr]);
	sync_ctrl->wr++;
	if (sync_ctrl->wr == CODA_META_NUM) {
		sync_ctrl->wr = 0;
	}
	sync_ctrl->cnt ++;
	return sync_block;
...
static struct sync_block* sync_search_addr(struct coda_sync_ctrl* sync_ctrl, guint addr)
{
	...
	while(sync_ctrl->rd != sync_ctrl->wr) {
		rd_idx = sync_ctrl->rd;

		sync_ctrl->rd++;
		if (sync_ctrl->rd == CODA_META_NUM) {
			sync_ctrl->rd = 0;
		}
		sync_ctrl->cnt --;
        if (sync_ctrl->cnt >0) {
            sync_ctrl->start_addr = sync_ctrl->block[sync_ctrl->rd].start_addr;
        }
		if ((addr >= sync_ctrl->block[rd_idx].start_addr && addr <= sync_ctrl->block[rd_idx].end_addr)
			||((sync_ctrl->block[rd_idx].start_addr > sync_ctrl->block[rd_idx].end_addr)
				&& (addr >= sync_ctrl->block[rd_idx].start_addr || addr <= sync_ctrl->block[rd_idx].end_addr)))
		{
            break;
		}
	}
	GST_LOG("rd_idx %x\n", (unsigned int)rd_idx);
	return &(sync_ctrl->block[rd_idx]);
}

static struct sync_block* cur_block(struct coda_sync_ctrl* sync_ctrl)
{
	if (sync_ctrl->cnt == 0) {
		return NULL;
	}
	if (sync_ctrl->wr == 0) {
	    return &(sync_ctrl->block[CODA_META_NUM-1]);
	} else {
...
static gint gst_coda_allocate_buffer(GstCodaDec *dec)
{
	...
	GetInfo->stride = lakers_frm_y_size =  GetInfo->picWidth;
	height = ( GetInfo->picHeight + 15 ) >> 4 << 4;		// 16 bytes aligned
	...
	dec->outsize = lakers_frm_y_size + 2 * lakers_frm_c_size;
	...
	gst_coda_dec_negotiate (dec, GetInfo->picWidth, GetInfo->picHeight);
	do{
...
static void VideoDecoder_Init(GstCodaDec * dec)
{
  struct vdec_private *ppriv;
  DecInstCtl *handle;
  guint vpu_addr;
  ...
  VPU_Init(&(ppriv->VPU_buffer), handle, 1);

  EnableVPUInt(handle, 1);
  ppriv->instIdx = 0;
  ...
  handle->SeqOpenParam.streamBufEndAddr = handle->SeqOpenParam.streamBufStartAddr + handle->SeqOpenParam.streamBufSize;
...
static void copydata2buffer(GstCodaDec * dec, guchar *data, guint size, GstClockTime pts)
{
    struct vdec_private *ppriv;
	Vbv_Ctrl *pvbv;
	struct coda_sync_ctrl	*sync_ctrl;  

    ppriv = (struct vdec_private *)dec->priv;
    pvbv = &ppriv->vbv_ctrl;
    sync_ctrl = &ppriv->sync_ctrl;

	while ((pvbv->end + 1- pvbv->start - pvbv->valid_size) < size) {
...
static gboolean StreamInBuf(GstCodaDec * dec)
{
  struct vdec_private *ppriv;
  DecInstCtl *handle;  
  Vbv_Ctrl *pvbv;
...
//	result = gst_pad_start_task (dec->sinkpad, (GstTaskFunction) gst_coda_dec_loop, dec->sinkpad);
	//pthread_create(&dec_thread, NULL, &gst_coda_dec_loop, dec);
  ...
  if ( ppriv->dec_status == SEQ_START_STATE || ppriv->dec_status == PICBIT_START_STATE){
	gst_coda_dec_frame(dec);
  }
  
  return TRUE;
}

static gboolean VideoStreamInput(GstCodaDec * dec, GstBuffer * buf)
{
  struct vdec_private *ppriv;
  DecInstCtl *handle;  
  Vbv_Ctrl *pvbv;
  ...
  if (ppriv->dec_status == IDLE_STATE || ppriv->dec_status == SEQ_START_STATE || ppriv->dec_status == SEQ_CONTINUE_STATE){
	readp = GetNewNormalReadPtr(handle, ppriv->instIdx, 1);
  }else{
	readp = GetNewReadPtr(handle, ppriv->instIdx, 1);
  }
#endif
  readp = GetNewNormalReadPtr(handle, ppriv->instIdx, 1);
	...
	if ( vbv_get_valid(pvbv_ctrl) ) {
		...
		if ( pvbv_ctrl->valid_size ){
			if ( VPU_DecInit(handle, ppriv->instIdx) == RETCODE_WRONG_CALL_SEQUENCE ){
				...
				//new_readp = GetNewReadPtr(handle, ppriv->instIdx, 1);
				new_readp = GetNewNormalReadPtr(handle, ppriv->instIdx, 1);
		...
		if ( handle->BitstreamFormat == STD_MJPG ){
			...
			if (!pfrm_ctrl->buffromv4l){
				//if ( (handle->SeqInfo.allUsedBuffers == VPU_GetDispFlag(handle, ppriv->instIdx)) || handle->FrameOutInfo.indexFrameDecoded == -1){	
					...
					do{
						//gst_coda_dec_negotiate (dec, handle->SeqInfo.picWidth, handle->SeqInfo.picHeight);
...
static void gst_coda_dec_stop(struct vdec_private *ppriv)
{
...
static void seq_init_isr(GstCodaDec *dec)
{
	struct vdec_private *ppriv;
	DecInstCtl *handle;  
...
//	cur_readp = GetNewReadPtr(handle, ppriv->instIdx, 1);
	cur_readp = GetNewNormalReadPtr(handle, ppriv->instIdx, 1);
	g_mutex_lock(dec->buflock);
...
static void seq_end_isr(GstCodaDec *dec)
{
	struct vdec_private *ppriv =(struct vdec_private *)dec->priv;
...
static void pic_run_isr(GstCodaDec *dec)
{
	struct vdec_private *ppriv =(struct vdec_private *)dec->priv;
	DecInstCtl *handle=ppriv->inst_handle;  
...
//	guchar ii, num;
	...
	if ( FrmOutInfo->indexFrameDecoded >= 0 ){
		if ( pfrm_ctrl->get_pts_valid == 1 ){
			pfrm_ctrl->frm_info[(guchar)FrmOutInfo->indexFrameDecoded].pts = pfrm_ctrl->get_pts;
			pfrm_ctrl->frm_info[(guchar)FrmOutInfo->indexFrameDecoded].pts_valid = 1;
			//PRINTF("indx %d PTS %x\n", FrmOutInfo->indexFrameDecoded, pfrm_ctrl->get_pts);
		}else{
			pfrm_ctrl->frm_info[(guchar)FrmOutInfo->indexFrameDecoded].pts_valid = 0;
...
//		printf("s%x, e%x,n%d\n", sync_ctrl->start_addr, sync_ctrl->end_addr, sync_ctrl->cnt);
	...
	cur_readp = GetNewNormalReadPtr(handle, ppriv->instIdx, 1);
	if ( FrmOutInfo->indexFrameDecoded >= 0 ){
		//printf("read pointer %x\n", GetNewReadPtr(handle, ppriv->instIdx, 1));
	}
	#if 0
	if (*(unsigned int*)(0x5F400144) == 0x34afdd3){
		/*
		FILE *fq, *ft;
		int a, b, addr;
		
		fq = g_fopen("errframe.bin", "wb");
		ft = g_fopen("errstream.bin", "wb");
		a = FrmOutInfo->indexFrameDecoded;
		addr = pfrm_ctrl->frm_info[a].frm_buffer.bufY;

		for ( a = 0; a < 600; a++ ){
			fwrite((gpointer)(hal_paddr_to_vaddr((gpointer)addr)), 720, 1, fq);
			addr+=720;
		}
		fclose(fq);
		addr = 0x3438754;
		fwrite((gpointer)(hal_paddr_to_vaddr((gpointer)addr)), 1626, 1, ft);
		fclose(ft);
		*/
	  	printf("pic writep %x, %x, %x, %x\n", pvbv_ctrl->config_end, pvbv_ctrl->writep, pvbv_ctrl->readp, pvbv_ctrl->valid_size);

		exit(0);
	}
	if (*(unsigned int*)(0x5F400144) == 0x34adc51){
		print_flag = 1;
	}
	#endif
	addr = hal_paddr_to_vaddr((gpointer)cur_readp);
	g_mutex_lock(dec->buflock);
	vbv_update_readp(pvbv_ctrl, cur_readp);
	if (pvbv_ctrl->vbv_vo_status == 1){
		sem_post(&dec->push_sem);
...
//	  printf ("total Execution %d frame ended after %lld ns.\n", frm_num, diff);
		...
		if ( FrmOutInfo->indexFrameDisplay != -2 && FrmOutInfo->indexFrameDisplay != -3 ) {		
			...
			pfrm_ctrl->frm_info[(guchar)FrmOutInfo->indexFrameDisplay].display = 1;
...
static void set_framebuffer_isr(GstCodaDec *dec)
{
	struct vdec_private *ppriv=(struct vdec_private *)dec->priv;
	...
	VPURegInit(ppriv->inst_handle, 0x180, 32, 1);
	ppriv->dec_status = PICFRM_START_STATE;
...
static void bsbuffer_underflow_isr(GstCodaDec *dec)
{
	struct vdec_private *ppriv=(struct vdec_private *)dec->priv;
//	DecInstCtl *handle=ppriv->inst_handle;  
...
static void
gst_coda_dec_negotiate (GstCodaDec * dec, gint width, gint height)
{
  GstCaps *caps;
  struct vdec_private *ppriv ;
  Frm_Ctrl *pfrm_ctrl;
...
static gboolean gst_coda_dec_isr(GstCodaDec *dec)
{
  struct vdec_private *ppriv;
  DecInstCtl *handle;  
  Vbv_Ctrl *pvbv_ctrl;
  ...
  status = GetVPUIntStatus(handle, 1);
...
//	  printf ("64bit Execution ended after %lld ns.\n", diff);
  ...
  if(status & VPU_ISR_SEQ_INIT) {	
	ClrVPUIntStatus(handle, status, VPU_ISR_SEQ_INIT, 1);
	ClrVPUBitInt(handle, 1);
	seq_init_isr(dec);
	if ( pvbv_ctrl->vbv_vo_status == 1 ){
  ...
  if(status & VPU_ISR_SEQ_END) {
	seq_end_isr(dec);
	ClrVPUIntStatus(handle, status, VPU_ISR_SEQ_END, 1);
	ClrVPUBitInt(handle, 1);
  }
...
//	double usedtime;
  ...
  if(status & VPU_ISR_SET_FRAME_BUFFER) {
	ClrVPUIntStatus(handle, status, VPU_ISR_SET_FRAME_BUFFER, 1);
	ClrVPUBitInt(handle, 1);
  ...
  if (status & VPU_ISR_BUF_FLUSH) {
	...
	ClrVPUIntStatus(handle, status, VPU_ISR_BUF_FLUSH, 1);
	ClrVPUBitInt(handle, 1);
  ...
  if (status & VPU_ISR_BUF_EMPTY) {
	ClrVPUIntStatus(handle, status, VPU_ISR_BUF_EMPTY, 1);
	ClrVPUBitInt(handle, 1);
...
static gboolean gst_coda_dec_status(DecInstCtl *dec_handle)
{
	if (VPU_IsBusy(dec_handle, 1))
		return FALSE;
...
static void gst_coda_dec_loop(void *param)
{
  GstCodaDec *dec = (GstCodaDec *)param;
  struct vdec_private *ppriv;
  DecInstCtl *handle;  
  Vbv_Ctrl *pvbv_ctrl;
  Frm_Ctrl *pfrm_ctrl;
/*
  ...
  printf("loop time %x%x\n", (guint32)((end-start)>>32)&0xFFFFFFFF, (guint32)(end-start)&0xFFFFFFFF);
  */
  //dec = GST_CODA_DEC (GST_OBJECT_PARENT (pad));
 ...
 // printf("dec loop 1\n");
  ...
  if (gst_coda_dec_status(handle) == TRUE )
  {
	gst_coda_dec_isr(dec);
  }
...
static gboolean
gst_coda_dec_src_query (GstPad * pad, GstQuery * query)
{
  gboolean res = TRUE;
  GstPad *peer;
  GstCodaDec *dec;

  dec = GST_CODA_DEC (GST_PAD_PARENT (pad));
...
static void
gst_coda_dec_init (GstCodaDec * dec)
{
  GstCodaDecClass *decclass;

  decclass = (GstCodaDecClass *) (G_OBJECT_GET_CLASS (dec));
  ...
  gst_pad_set_setcaps_function (dec->sinkpad, GST_DEBUG_FUNCPTR (gst_coda_dec_setcaps));
  gst_pad_set_chain_function (dec->sinkpad, GST_DEBUG_FUNCPTR (gst_coda_dec_chain));
  gst_pad_set_event_function (dec->sinkpad, GST_DEBUG_FUNCPTR (gst_coda_dec_sink_event));

  dec->srcpad = gst_pad_new_from_static_template (&src_template, "src");
  gst_pad_set_event_function (dec->srcpad, GST_DEBUG_FUNCPTR (gst_coda_dec_src_event));
  gst_pad_use_fixed_caps (dec->srcpad);
  gst_pad_set_query_function (dec->srcpad, GST_DEBUG_FUNCPTR (gst_coda_dec_src_query));  
  gst_element_add_pad (GST_ELEMENT (dec), dec->srcpad);

  
  dec->buflock = g_mutex_new ();
  
  /* setup coda decoder lib */
  VideoDecoder_Init(dec);

  dec->fd = hal_device_attach("vdec_coda");

  hal_int_register(dec->fd, gst_coda_dec_loop, (void *)dec);
...
static gboolean
gst_coda_dec_setcaps (GstPad * pad, GstCaps * caps)
{
  GstStructure *s;
  GstCodaDec *dec;
...
//  DecInstCtl *handle;  
  ...
  gst_coda_caps_to_codecid(dec, caps);
...
gst_coda_dec_chain (GstPad * pad, GstBuffer * buf)
{
//  GstFlowReturn ret = GST_FLOW_OK;
  GstCodaDec *dec;
//  struct vdec_private *ppriv;
...
static gboolean
gst_coda_dec_src_event (GstPad * pad, GstEvent * event)
{
  GstCodaDec *dec;
  gboolean res;
  ...
  dec = GST_CODA_DEC (gst_pad_get_parent (pad));
...
//      gst_coda_dec_update_qos (dec, proportion, diff, timestamp);
...
static gboolean
gst_coda_dec_sink_event (GstPad * pad, GstEvent * event)
{
  gboolean ret = TRUE;
  GstCodaDec *dec = GST_CODA_DEC (GST_OBJECT_PARENT (pad));
  struct vdec_private *ppriv = (struct vdec_private *)dec->priv;  
  DecInstCtl *handle;  
//  Vbv_Ctrl *pvbv;  
//  guint intmask;
...
#if 0
static gboolean gst_coda_thread(GstCodaDec *dec)
{
  GThread *thread;
  GError *error = NULL;

  thread =
     g_thread_create ((GThreadFunc) gst_coda_dec_loop, dec, TRUE, &error);
  if (error != NULL) {
...
gst_coda_dec_change_state (GstElement * element, GstStateChange transition)
{
  GstStateChangeReturn ret;
  GstCodaDec *dec;

  dec = GST_CODA_DEC (element);
...
#if 0
	  if ( dec->task == NULL ){
	    dec->task = gst_task_create ((GstTaskFunction) gst_coda_dec_loop, dec);
		gst_task_set_priority (dec->task, G_THREAD_PRIORITY_URGENT);
	    gst_task_set_lock (dec->task, GST_CODA_GET_LOCK(dec));
	  }
	 #endif
	  //gst_task_start (dec->task);
