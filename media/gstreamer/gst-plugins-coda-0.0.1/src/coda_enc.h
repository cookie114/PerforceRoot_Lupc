...
#define VENC_REG_BASE_ADDR      0x1F400000
#define VENC_MEM_BASE_ADDR      0x03200000
#define VENC_STREAM_BUF_SIZE    0x200000
#define VENC_FRAME_BUF_SIZE     0x3000000
#define VENC_MV_BUF_ADDR        0x13600000
#define VENC_FRAME_BUF_ADDR		0X10000000
...
#define CODA_META_NUM	500
struct coda_sync_ctrl {
	struct sync_block block[CODA_META_NUM];
	gushort rd;
	gushort wr;
	gushort cnt;
...
struct venc_start_param {
	guchar  enc_std;
};

struct venc_private{
	guchar					instIdx;
	EncInstCtl *			inst_handle;

	enum coda_enc_status	enc_status;
	enum coda_enc_status    stop_status;
	struct coda_frm_ctrl	frm_ctrl;
	struct coda_vbv_ctrl	vbv_ctrl;
	struct coda_sync_ctrl	sync_ctrl;
	struct frame_info		output_video_info;
	guchar					stream_update;
	EncConfigParam          ConfParam;

	guint					disp_cnt;
	guchar                  encoded_first_frame;
