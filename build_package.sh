...
gzip -q $tmp_dir/data.tar
tar -C $pkg_dir/$CONTROL -cf $tmp_dir/control.tar .
gzip -q $tmp_dir/control.tar
...
# Follow code added for match LUPC/Relase rule
if [ ! -z "${DEFINE_PKG_ROOT}" ]
then
	# Append new section into Packages file
	pkg_md5=$(md5sum -b ${pkg_file} | cut -d " " -f1)
	pkg_len=$(stat -c %s ${pkg_file})
	pkg_name=$(basename ${pkg_file})
	cat  ${pkg_dir}/${CONTROL}/control	>> ${DEFINE_PKG_ROOT}/Packages
	echo "Filename: ${pkg_name}"		>> ${DEFINE_PKG_ROOT}/Packages
	echo "Size: ${pkg_len}"				>> ${DEFINE_PKG_ROOT}/Packages
	echo "MD5Sum: ${pkg_md5}"			>> ${DEFINE_PKG_ROOT}/Packages
	echo								>> ${DEFINE_PKG_ROOT}/Packages
	# Generate Packages.gz
	gzip -c ${DEFINE_PKG_ROOT}/Packages > ${DEFINE_PKG_ROOT}/Packages.gz
	mv -f $pkg_file ${DEFINE_PKG_ROOT}
fi
